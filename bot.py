...
class DispatchView(discord.ui.View):
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.set_author(name=f"Dispatched by: {self.author_display_name}", icon_url=self.author_avatar_url)
        embed.set_footer(text="Bot Concept Credits: @MOOO")
...
class ConfirmLandedView(discord.ui.View):
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name="\u200b", value=f"**📝 Notes:** {self.flight_data.get('notes', 'No notes')}", inline=False)
        embed.set_author(name=f"Dispatched by: {self.author_display_name}", icon_url=self.author_avatar_url)
        embed.set_footer(text="Bot Concept Credits: @MOOO | Flight Completed")
...
class FlightDetailsModal(discord.ui.Modal, title="Flight Details"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed.set_author(name=f"Dispatched by: {interaction.user.display_name}", icon_url=interaction.user.display_avatar.url)
        embed.set_footer(text="Bot Concept Credits: @MOOO")
...
async def live(interaction: discord.Interaction):
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            ...
            # Get sessions
            async with session.get(f"{IF_API_BASE}/sessions", headers=headers) as resp:
                data = await resp.json()
                sessions = data.get("result", [])
                expert_id = None
                for s in sessions:
                    if s.get("worldType") == 3:
                        expert_id = s.get("id")
                        break
            ...
            # Get flights
            async with session.get(f"{IF_API_BASE}/sessions/{expert_id}/flights", headers=headers) as resp:
                data = await resp.json()
            ...
            if not me_flights:
                await interaction.followup.send("✈️ No MEAV flights active right now")
                return
            ...
            for flight in me_flights:
                ...
                route_text = f"{dep_icao} → {arr_icao}"                
